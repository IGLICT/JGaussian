...
class densification_module(jt.nn.Module):
    
    def __init__(self, num_points):
        super().__init__()
        
        # TODO save desification relevant tensors?
        self.xyz_gradient_accum = jt.zeros((num_points, 1))
        self.denom = jt.zeros((num_points, 1))
        self.max_radii2D = jt.zeros((num_points))
        self.opacity_accum = jt.zeros((num_points, 1))
    
        self.scaling_activation = jt.exp
        self.scaling_inverse_activation = jt.log
        self.opacity_activation = jt.sigmoid
        self.rotation_activation = jt.nn.normalize
    ...
    def densification_postfix(self, new_xyz, new_features_dc=None, new_features_rest=None, new_opacities=None, new_scaling=None, new_rotation=None):
        ...
        self.opacity_accum = jt.zeros((self.get_xyz.shape[0], 1))
    ...
    #     selected_pts_mask = jt.logical_and(dist > (5. * scene_extent),
    #                                           jt.max(self.get_scaling, dim=1).values > (scene_extent))
    ...
    #     new_rotation = jt.zeros_like(self._rotation[new_indices])
    #     new_rotation[:, 0] = 1
    #     new_features_dc = jt.zeros_like(self._features_dc[new_indices])
    #     new_features_rest = jt.zeros_like(self._features_rest[new_indices])
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        prune_filter = jt.cat((selected_pts_mask, jt.zeros(N * selected_pts_mask.sum(), device=self._xyz.device, dtype=bool)))
        
        # TODO opacity_accum also needs to opacity accumulation
        self.opacity_accum_cur = jt.cat([self.opacity_accum_cur[jt.logical_not(selected_pts_mask)], \
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        dists, indices = jt.sort(dist, dim=-1)
        ...
        new_xyz = self._xyz[selected_pts_mask] + jt.randn_like(self._xyz[selected_pts_mask]) * 0.03
        ...
        # TODO opacity_accum also needs to opacity accumulation
        self.opacity_accum_cur = jt.cat([self.opacity_accum_cur] + [self.opacity_accum_cur[selected_pts_mask]] * K, dim=0)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, opacity, densify_inteval, cur_pts):
        ...
        if type(viewspace_point_tensor) == list:
            ...
            for vpt, upfilter in zip(viewspace_point_tensor, update_filter):
                viewpoint_grads += jt.norm(vpt.grad[:,:2] * upfilter.unsqueeze(-1), dim=-1, keepdim=True) / len(viewspace_point_tensor)
    ...
    def add_densification_stats_maxgrad(self, viewspace_point_tensor, update_filter, opacity, densify_inteval, cur_pts):
        ...
        if type(viewspace_point_tensor) == list:
            ...
            # for vpt, upfilter in zip(viewspace_point_tensor, update_filter):
            #     viewpoint_grads = jt.norm(vpt.grad[:,:2] * upfilter.unsqueeze(-1), dim=-1, keepdim=True)
            #     upfilter_agg += upfilter.unsqueeze(-1)
            upfilter = jt.stack(update_filter).unsqueeze(-1)
            upfilter_agg = jt.sum(upfilter, dim=0)
            viewpoint_grads = jt.norm(jt.stack(viewspace_point_tensor).grad[:, :,:2] * upfilter, dim=-1, keepdim=True).max(0)
        ...
        #     print(xyz_grads.mean(), xyz_grads.std(), xyz_grads.max(), xyz_grads.min())
        # self.xyz_gradient_accum += xyz_grads
        self.xyz_gradient_accum = jt.max(self.xyz_gradient_accum, xyz_grads)
        self.denom = 1
        # self.denom += 1
    ...
    def add_densification_stats_original(self, viewspace_point_tensor, update_filter, opacity, densify_inteval, cur_pts):
        ...
        if type(viewspace_point_tensor) == list:
            for vpt in viewspace_point_tensor:
                self.xyz_gradient_accum[update_filter] += jt.norm(vpt.grad[update_filter,:2], dim=-1, keepdim=True) / len(viewspace_point_tensor)
        else:
