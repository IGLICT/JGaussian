...
from jittorutils import bias_act
# import jt.nn.init as init
from jittor import init
from training.gaussian3d_splatting.sh_utils import RGB2SH

# import jt.nn.functional as F
from training.gaussian3d_splatting.sh_utils import C0
...
class ModulatedFullyConnectedLayer(jt.nn.Module):
    def __init__(self,
        ...
        self.weight = jt.randn([out_features, in_features]) / lr_multiplier * weight_init
        self.bias = jt.full([out_features], np.float32(bias_init)) if bias else None
        ...
        if residual:
            ...
            self.layerscale = jt.ones([1, 1, out_features]) * 1e-2

    def execute(self, x, w, demodulate=True):
        # x: [B, L, C] / w: [B, w_dim]
        ...
        if x.dtype == jt.float16 and demodulate:
        ...
        if demodulate:
            ...
            dcoefs = (weight_styled.pow(2).sum(dim=(2)) + 1e-8).rsqrt() # [NO]
...
class PlainMLP(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(width=width, init_scale=None)
        self.ls = jt.ones([width]) * 0
    
    def execute(self, x, w=None):
...
class LinLinear(nn.Module):
    def __init__(self, ch_in, ch_out, is_first=False, bias=True):
        ...
        if is_first:
            init.uniform_(self.linear.weight, -np.sqrt(9 / ch_in), np.sqrt(9 / ch_in))
        else:
            init.uniform_(self.linear.weight, -np.sqrt(3 / ch_in), np.sqrt(3 / ch_in))
...
class constant_PE(nn.Module):
    def __init__(self, pe_dim, pe_res):
        ...
        # constant PE
        self.const_pe = jt.randn([3, pe_dim, 1, pe_res])
    ...
    def execute(self, pos):
        ...
        x_coord = jt.cat([jt.zeros_like(x_coord), x_coord], dim=-1) # [B * 3, 1, L, 2]
        
        const_pe = self.const_pe.repeat([B, 1, 1, 1]) # [B * 3, C, 1, L]
        const_emb = jt.nn.grid_sample(const_pe, x_coord, mode='bilinear') # [B * 3, C, 1, L]
...
@persistence.persistent_class
class PointUpsample_subpixel(jt.nn.Module):
    def __init__(self,
        in_features,                # Number of input features.
        out_features,               # Number of output features.
        upsample_ratio,
    ...
    def execute(self, x, pc, w=None, pe_data=None):
        # x: [B, L, C], 
...
@persistence.persistent_class
class PointUpsample_modsubpixel(jt.nn.Module):
    def __init__(self,
        in_features,                # Number of input features.
        out_features,               # Number of output features.
        upsample_ratio,
    ...
    def execute(self, x, pc, w, pe_data=None):
        # x: [B, L, C], 
        # pc: [xyz, scale, rotation, color, opacity]
        B, L, C = x.shape
...
@persistence.persistent_class
class CoordInjection_const(jt.nn.Module):
    def __init__(self,
    ...
    def execute(self, pos, x=None, type='cat'):
        if x is not None:
            x = jt.cat([x, self.learnable_pe(pos).to(x.dtype), self.const_pe(pos).to(x.dtype)], dim=-1)
        else:
            x = jt.cat([self.learnable_pe(pos), self.const_pe(pos)], dim=-1)
        return x
...
def get_edge_features(x, k, num=-1, idx=None, return_idx=False):
    ...
    if idx is None:
        xt = x.permute(0, 2, 1)
        xi = -2 * jt.bmm(xt, x)
        xs = jt.sum(xt**2, dim=2, keepdim=True)
        xst = xs.permute(0, 2, 1)
        dist = xi + xs + xst # [B, N, N]

        # get k NN id
        _, idx_o = jt.sort(dist, dim=2)
    ...
    for b in range(B):
        ...
        # tmp = jt.index_select(x[b], 1, idx[b]) # [d, N*k] <- [d, N], 0, [N*k]
        tmp = x[b][:, idx[b]]
 
        tmp = tmp.view(dims, N, k)
        neighbors.append(tmp)

    neighbors = jt.stack(neighbors) # [B, d, N, k]

    # centralize
    central = x.unsqueeze(3) # [B, d, N, 1]
    central = central.repeat(1, 1, 1, k) # [B, d, N, k]

    ee = jt.cat([central, neighbors-central], dim=1)
...
class AdaEdgeBlock(nn.Module):
    ...
    def execute(self, x, w=None, pos=None, upsample_factor=None):
        # x: [B, L, C]
        x_original = x
        ...
        w_softmax = nn.softmax(w_softmax.to(jt.float32), dim=-1)  # [B, Fout, N, k] -> [B, Fout, N, k]
...
class LocalEdgeBlock(nn.Module):
    ...
    def execute(self, x, w=None, pos=None, upsample_factor=None):
        ...
        x = jt.cat([x_center, x_delta], dim=1) # [B, 2C, L, ups]
        
        w = self.conv_w(x[:, C:, :, :])
        w = nn.softmax(w.to(jt.float32), dim=-1)  # [B, Fout, N, k] -> [B, Fout, N, k]
        w = w.to(x.dtype)
...
class PointGenerator(nn.Module):
    def __init__(self,
        ...
        _out_keys = {"xyz": [3, 1.0, 1.0], 
                     ...
                     "opacity": [1, 1, 1.],
                     "albedo": [img_channels, 1, 1.], 
                     "dc_rt": [l_dim, 0.1, 1.],
                     }
        
        self._out_keys = _out_keys
        for k in _out_keys.keys():
            out_layers[k] = OrderedDict() # nn.ModuleDict()
        ...
        while res_start < img_resolution:
            ...
            blocks[f"block_{i}"] = nn.ModuleList([])
            # self.blocks[f"block_{i}"] = nn.ModuleList([])
            out_norm[f"block_{i}"] = Identity()
            pe_dim = 0
            if i > self.upsample_res:
                ...
                if num_pts_start <= init_pts * self.upsample_ratio ** 2: # leq level 3
                    blocks[f"block_{i}"].append(AdaEdgeBlock(cur_in, cur_in, k=knn)) # TODO
                    self.num_ws += 1
                else:
                    blocks[f"block_{i}"].append(LocalEdgeBlock(cur_in, cur_in))
                    self.num_ws += 0
                    
                # Upsample layer
                blocks[f"block_{i}"].append(PointUpsample_subpixel(in_features=cur_in, out_features=cur_out, pe_dim=pe_dim, \
                                            upsample_ratio=self.upsample_ratio, resolution=pe_res))

                if num_pts_start <= init_pts * self.upsample_ratio ** 2: # leq level 3
                    blocks[f"block_{i}"].append(AdaMLP(width=cur_out))
                    self.num_ws += 1
                else:
                    blocks[f"block_{i}"].append(PlainMLP(width=cur_out))
                    self.num_ws += 0
                    
            else:
                # Transformer layer
                self.n_transformer = 6
                blocks[f"block_{i}"].append(Transformer(width=cur_in, layers=self.n_transformer))
            ...
            for k in _out_keys.keys():
                out_layers[k][f"block_{i}"] = nn.ModuleList([])
                
                # Anchor gaussian layer
                out_layers[k][f"block_{i}"].append(linear_op(cur_out, _out_keys[k][0], lr_multiplier=_out_keys[k][2], activation="linear", weight_init=_out_keys[k][1]))
                ...
                cur_in_channel = cur_out
                out_layers[k][f"block_{i}"].append(linear_op(cur_in_channel, _out_keys[k][0] * self.output_multiplier, lr_multiplier=_out_keys[k][2], activation="linear", weight_init=_out_keys[k][1]))
        ...
        # self.register_buffer("scale_init", jt.ones([3]) * options['scale_init'])
        # self.register_buffer("scale_threshold", jt.ones([3]) * options['scale_threshold'])
        # self.register_buffer("rotation_init", jt.array([1, 0, 0, 0]))
        # self.register_buffer("albedo_init", jt.array(jt.zeros([img_channels])))
        # self.register_buffer("opacity_init", inverse_sigmoid(0.1 * jt.ones([1])))
        # self.register_buffer("dc_rt_init", jt.zeros([l_dim]))
        self.scale_init = jt.ones([3]).stop_grad() * options['scale_init']
        self.scale_threshold = jt.ones([3]).stop_grad() * options['scale_threshold']
        self.rotation_init = jt.array([1, 0, 0, 0]).stop_grad()
        self.albedo_init = jt.zeros([img_channels]).stop_grad()
        self.opacity_init = inverse_sigmoid(0.1 * jt.ones([1])).stop_grad()
        self.dc_rt_init = jt.zeros([l_dim]).stop_grad()
        ...
        for k in _out_keys.keys():
            out_layers[k] = nn.ModuleList(out_layers[k]) # nn.ModuleDict()
        self.blocks = nn.ModuleList(blocks)
        self.out_norm = nn.ModuleList(out_norm)
        self.out_layers = nn.ModuleList(out_layers)

    def execute(self, x, ws, camera_cond=None, camera_swap_p=0.0):
        # x: [B, L, C]
        B, L, C = x.shape
        pos_sphere = x
        ...
        if len(ws.shape) == 2:
            ws = ws.unsqueeze(1).repeat(1, self.num_ws, 1) # ws: [B, num_ws, w_dim]
        ...
        if self.use_dir_cond:
            ...
            if camera_swap_p > 0:
                camera_cond_swapped = jt.roll(camera_cond.clone(), 1, 0)
                camera_cond = jt.where(jt.rand((camera_cond.shape[0], 1)) < camera_swap_p, camera_cond_swapped, camera_cond)
            camera_cond = self.encoder_camera_cond(camera_cond)
        
        xyz_prev, scale_prev, rotation_prev, albedo_prev, opacity_prev, dc_rt_prev = 0, self.scale_init, self.rotation_init, self.albedo_init, self.opacity_init, self.dc_rt_init
        ...
        for i in self.block_res:
            ...
            if i > self.upsample_res:
                ...
                x, (xyz_prev, scale_prev, rotation_prev, albedo_prev, opacity_prev, dc_rt_prev) = upsample(x, [xyz_prev, scale_prev, rotation_prev, albedo_prev, opacity_prev, dc_rt_prev], w=w_up)
            ...
            if i >= self.res_fp16_start:
                out = out.to(jt.float32)
            
            # Last layer (do not generator anchor gaussians)
            if i == self.block_res[-1]:
                
                # Point generation part
                pc = [self.out_layers[k][f"block_{i}"][1](out) for k in ["xyz", "scale", "rotation", "albedo", "opacity", "dc_rt"]]
                
                # if self.use_dir_cond:
                #     out_color = jt.cat([out, camera_cond.unsqueeze(1).repeat(1, out.shape[1], 1)], dim=-1)
                # else:
                #     out_color = out
                # pc.insert(3, self.out_layers['color'][f"block_{i}"][1](out_color))
                
                pc_prev = [xyz_prev, scale_prev, rotation_prev, albedo_prev, opacity_prev, dc_rt_prev]
                
                # Postprocessing (densify)
                xyz_cur, scale_cur, rotation_cur, albedo_cur, opacity_cur, dc_rt_cur = self.postprocessing_block(pc, pc_prev, percent_dense=self.percent_dense, \
                ...
                albedo = jt.cat([albedo, albedo_cur], dim=1)
                opacity = jt.cat([opacity,  opacity_cur], dim=1)
                dc_rt = jt.cat([dc_rt, dc_rt_cur], dim=1)

            # Intermediate layers (generate both anchor and output gaussians)
            elif i > self.upsample_res:
                pc = [self.out_layers[k][f"block_{i}"][0](out) for k in ["xyz", "scale", "rotation", "albedo", "opacity", "dc_rt"]]
                pc_prev = [xyz_prev, scale_prev, rotation_prev, albedo_prev, opacity_prev, dc_rt_prev]
                ...
                # Point generation part
                pc_cur_out = [self.out_layers[k][f"block_{i}"][1](out) for k in ["xyz", "scale", "rotation", "albedo", "opacity", "dc_rt"]]
                ...
                if self.output_multiplier > 1:
                    ...
                    for _comp, _comp_prev in zip(pc_cur_out, pc_prev):
                        ...
                        pc_prev_new.append(jt.repeat_interleave(_comp_prev, self.output_multiplier, dim=1))
                    pc_cur_out, pc_prev = pc_new, pc_prev_new
                
                # Postprocessing (densify)
                xyz_cur_out, scale_cur_out, rotation_cur_out, albedo_cur_out, opacity_cur_out, dc_rt_cur_out = self.postprocessing_block(pc_cur_out, pc_prev, percent_dense=self.percent_dense, \
                                                                            N=self.upsample_ratio, split_ratio=self.split_ratio, \
                                                                            scale_upsample_reg=self.scale_out_delta)
                
                xyz = jt.cat([xyz, xyz_cur_out], dim=1)
                scale = jt.cat([scale, scale_cur_out], dim=1)
                rotation = jt.cat([rotation, rotation_cur_out], dim=1)
                albedo = jt.cat([albedo, albedo_cur_out], dim=1)
                opacity = jt.cat([opacity, opacity_cur_out], dim=1)
                dc_rt = jt.cat([dc_rt, dc_rt_cur_out], dim=1)
                                
                xyz_anchor = jt.cat([xyz_anchor, xyz_cur], dim=1)
                scale_anchor = jt.cat([scale_anchor, scale_cur], dim=1)
                rotation_anchor = jt.cat([rotation_anchor, rotation_cur], dim=1)
                albedo_anchor = jt.cat([albedo_anchor, albedo_cur], dim=1)
                opacity_anchor = jt.cat([opacity_anchor, opacity_cur], dim=1)
                dc_rt_anchor = jt.cat([dc_rt_anchor, dc_rt_cur], dim=1)
            ...
            # First layer (does not have previous anchors and upsamples, so generate both of them)
                ...
                albedo_cur = albedo_prev + self.out_layers["albedo"][f"block_{i}"][0](out)
                opacity_cur = opacity_prev + self.out_layers["opacity"][f"block_{i}"][0](out)
                dc_rt_cur = dc_rt_prev + self.out_layers["dc_rt"][f"block_{i}"][0](out)
                ...
                # out_color = jt.cat([out, camera_cond.unsqueeze(1).repeat(1, out.shape[1], 1)], dim=-1) if self.use_dir_cond else out
                ...
                albedo_cur_out = albedo_prev + self.out_layers["albedo"][f"block_{i}"][1](out).reshape(B, L * self.output_multiplier, -1)
                opacity_cur_out = opacity_prev + self.out_layers["opacity"][f"block_{i}"][1](out).reshape(B, L * self.output_multiplier, -1)
                dc_rt_cur_out = dc_rt_prev + self.out_layers["dc_rt"][f"block_{i}"][1](out).reshape(B, L * self.output_multiplier, -1)
                
                
                # Postprocessing (densify)
                if i == self.upsample_res:
                    xyz_cur = jt.tanh(xyz_cur)
                    scale_cur = - jt.nn.softplus(- (scale_cur - self.scale_threshold)) + self.scale_threshold

                    if self.output_multiplier > 1:
                        rotation_cur_temp = jt.repeat_interleave(rotation_cur, self.output_multiplier, dim=1)
                        scale_cur_temp = jt.repeat_interleave(scale_cur, self.output_multiplier, dim=1)
                        xyz_cur_temp = jt.repeat_interleave(xyz_cur, self.output_multiplier, dim=1)
                    else:
                        rotation_cur_temp = rotation_cur
                        scale_cur_temp = scale_cur
                        xyz_cur_temp = xyz_cur

                    samples = jt.tanh(xyz_cur_out) # [B, L * N, 3]
                    ...
                    scale_cur_out = - jt.nn.softplus(- (scale_cur_out - self.scale_threshold - self.scale_out_delta)) + self.scale_threshold + self.scale_out_delta
                    
                    
                # TODO check xyz, ... is output of point generator
                xyz_anchor, scale_anchor, rotation_anchor, albedo_anchor, opacity_anchor, dc_rt_anchor = xyz_cur, scale_cur, rotation_cur, albedo_cur, opacity_cur, dc_rt_cur
                xyz, scale, rotation, albedo, opacity, dc_rt = xyz_cur_out, scale_cur_out, rotation_cur_out, albedo_cur_out, opacity_cur_out, dc_rt_cur_out
                
            # TODO check xyz_prev, ... is point clouds using for next layer
            # update previous gaussian params
            xyz_prev, scale_prev, rotation_prev, albedo_prev, opacity_prev, dc_rt_prev = xyz_cur, scale_cur, rotation_cur, albedo_cur, opacity_cur, dc_rt_cur
                
        # Output phase
        B, L, _ = xyz.shape
        xyz, scale, rotation, albedo, opacity, dc_rt = xyz.view(B, L, -1), scale.view(B, L, -1), \
                                            rotation.view(B, L, -1), albedo.view(B, L, -1), opacity.view(B, L, -1), dc_rt.view(B, L, -1)
        xyz = jt.clamp(xyz, -1, 1)

        # albedo = bias_act.bias_act(albedo, act='sigmoid')
        # albedo_anchor = bias_act.bias_act(albedo_anchor, act='sigmoid')

        # color = bias_act.bias_act(color, act='sigmoid')
        # color_anchor = bias_act.bias_act(color_anchor, act='sigmoid')

        # color = (color - 0.5) * 2 * (0.5 / C0)
        
        return xyz, scale, rotation, albedo, opacity, dc_rt, [xyz_anchor, scale_anchor, rotation_anchor, albedo_anchor, opacity_anchor, dc_rt_anchor]
    

    def postprocessing_block(self, pc, pc_prev, percent_dense=0.05, N=2, split_ratio=None, scale_upsample_reg=0):
        xyz, scale, rotation, albedo, opacity, dc_rt = pc_prev # previous outputs
        xyz_cur, scale_cur, rotation_cur, albedo_cur, opacity_cur, dc_rt_cur = pc # current outputs
    
        rotation_new = rotation + rotation_cur
        # color_new = color + color_cur * self.color_output_scale
        albedo_new = albedo + albedo_cur
        opacity_new = opacity + opacity_cur
        dc_rt_new = dc_rt + dc_rt_cur
        ...
        # remove outliers
        scale_new = jt.clamp(scale_new, -8, 0)
        
        pc_upsampled = [xyz_new, scale_new, rotation_new, albedo_new, opacity_new, dc_rt_new]
        return pc_upsampled
    
    
    def mutiply_quaternion(self, q1, q2):
        q1, q2 = jt.nn.normalize(q1, dim=-1), jt.nn.normalize(q2, dim=-1)
        ...
        q_new = jt.stack([w_product, x_product, y_product, z_product], dim=-1)
        return jt.nn.normalize(q_new, dim=-1)
