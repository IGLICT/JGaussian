...
class LookAtPoseSampler:
    ...
    Example:
    For a camera pose looking at the origin with the camera at position [0, 0, 1]:
    cam2world = LookAtPoseSampler.sample(math.pi/2, math.pi/2, jt.tensor([0, 0, 0]), radius=1)
    """
...
class CustomCameraPoseSampler:
    ...
    @staticmethod
    def sample(horizontal_mean, vertical_mean, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu', vertical_type='uniform', horizontal_type='gaussian'):
...
def create_cam2world_matrix(forward_vector, origin):
    ...
    up_vector = jt.array([0, 1, 0], dtype=jt.float).expand_as(forward_vector)
