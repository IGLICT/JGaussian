...
class InstanceNorm1d(nn.Module):
    ...
    def execute(self, x):
        ...
        x = (x - x.mean(1, keepdim=True)) / (jt.sqrt(x.var(dim=1, keepdims=True, unbiased=False)) + 1e-8)
...
def pixel_norm(x: jt.Var, eps=1e-4, dim=1):
    return x / jt.sqrt(jt.mean(x ** 2, dim=dim, keepdim=True) + eps)
...
class QKVMultiheadAttention(nn.Module):
    ...
    def execute(self, qkv):
        ...
        weight = jt.nn.softmax(weight.float(), dim=-1).to(wdtype)
        return jt.einsum("bhts,bshc->bthc", weight, v).reshape(bs, n_ctx, -1)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def execute(self, x: jt.Var, w: jt.Var):
...
class Transformer(nn.Module):
    ...
    def execute(self, x: jt.Var, ws: jt.Var):
